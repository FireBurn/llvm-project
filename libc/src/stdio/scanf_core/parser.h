...
public:
  ...
  LIBC_INLINE FormatSection get_next_section() {
    ...
    if (str[cur_pos] == '%') {
      ...
      // The allocating conversion, where what is written is not the string
      // but a pointer to storage this allocates for it.
      if (str[cur_pos] == 'm') {
        ++cur_pos;
        section.flags =
            static_cast<FormatFlags>(section.flags | FormatFlags::ALLOCATE);
      }
