...
struct group entry;
char line[grp::GROUP_LINE_SIZE];
} // anonymous namespace

// Reads one entry from a stream the caller opened, rather than from the
// group file. Blank lines and comments are passed over.
LLVM_LIBC_FUNCTION(struct group *, fgetgrent, (::FILE * stream)) {
  if (stream == nullptr) {
    libc_errno = EINVAL;
    return nullptr;
  }
