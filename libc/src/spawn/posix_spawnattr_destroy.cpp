//===-- Implementation of posix_spawnattr_destroy ----------===//
//
...
LLVM_LIBC_FUNCTION(int, posix_spawnattr_destroy, (posix_spawnattr_t * attr)) {
  if (attr == nullptr)
    return EINVAL;
  // There is nothing held on to, so there is nothing to give back.
  return 0;
}
