//===-- The path search execvp and execvpe share ----------------*- C++ -*-===//
//
...
#ifndef LLVM_LIBC_SRC_UNISTD_LINUX_EXEC_PATH_SEARCH_H
#define LLVM_LIBC_SRC_UNISTD_LINUX_EXEC_PATH_SEARCH_H
...
LIBC_INLINE int exec_at(const char *path, char *const argv[],
                        char *const envp[]) {
  return LIBC_NAMESPACE::syscall_impl<int>(SYS_execve, path, argv, envp);
}

// Runs |file| by searching PATH for it, with |envp| as the environment.
// Only returns on failure, with errno set.
LIBC_INLINE int run(const char *file, char *const argv[], char *const envp[]) {
  ...
  if (internal::find_first_character(
    ...
    int ret = exec_at(file, argv, envp);
    libc_errno = -ret;
    return -1;
  }
  ...
  for (const char *segment = path; segment != nullptr;) {
    ...
    if (prefix + 1 + file_length + 1 <= sizeof(candidate)) {
      ...
      const int error = -exec_at(candidate, argv, envp);
      if (error == EACCES) {
        saw_eacces = true;
...
#endif // LLVM_LIBC_SRC_UNISTD_LINUX_EXEC_PATH_SEARCH_H
