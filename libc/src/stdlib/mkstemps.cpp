...
///
/// \file
/// mkstemp for a template whose name ends in a suffix to keep.
///
//===----------------------------------------------------------------------===//
...
LLVM_LIBC_FUNCTION(int, mkstemps, (char *tmpl, int suffix_length)) {
  ...
  int open_flags = (0) & ~(O_ACCMODE | O_CREAT | O_EXCL);
